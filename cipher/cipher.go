...
type salsaStreamcipherr struct {
	nonce   [8]byte
	key     [32]byte
	counter int
}

func (c *salsaStreamcipherr) XORKeyStream(dst, src []byte) {
	var buf []byte
	padLen := c.counter % 64
...
func CheckcipherrMethod(method string) error {
	if method == "" {
		method = "aes-256-cfb"
	}
...
func NewCipher(method, password string) (c Cipher, err error) {
	...
	c = &cipherr{key: key, info: mi}
	return c, nil
}

// Initializes the block cipher with CFB mode, returns IV.
func (c *cipherr) initEncrypt() (iv []byte, err error) {
...
func (c *cipherr) initDecrypt(iv []byte) (err error) {
	c.dec, err = c.info.newStream(c.key, iv, Decrypt)
	return
}

func (c *cipherr) Encrypt(src []byte) (dst []byte) {
	c.Lock()
	defer c.Unlock()
	iv, _ := c.initEncrypt()
	l := len(src) + len(iv)
	...
	copy(cipherData, iv)
	c.enc.XORKeyStream(cipherData[len(iv):], src)
	return cipherData
}

func (c *cipherr) Decrypt(src []byte) (dst []byte) {
	c.Lock()
	defer c.Unlock()
	n := len(src)
	...
	c.initDecrypt(iv)
	c.dec.XORKeyStream(dst[0:], src[c.info.ivLen:n])
	return dst
}

// Copy creates a new cipher at it's initial state.
func (c *cipherr) Copy() *cipherr {
	// This optimization maybe not necessary. But without this function, we
	// need to maintain a table cache for newTablecipherr and use lock to
